...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        ...
        //return accounts.stream().collect(toMap(a-> a.getSex().equals(Sex.MALE), Collections::singletonList));
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream().anyMatch(a -> a.getEmail().equals(emailDomain));
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        Optional<Account> accountOptional = accounts.stream().filter(a -> a.getEmail().equals(email)).findFirst();
        if (accountOptional.isPresent())
            return accountOptional.get().getBalance();

        throw new EntityNotFoundException(String.format("Cannot find Account by email={%s}", email));
    }
    ...
    public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
        return accounts.stream().filter(a -> a.getCreationDate().getYear() == year).collect(toMap(Account::getEmail, Account::getBalance));
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        //return accounts.stream().collect(groupingBy(a -> a.getCreationDate().getMonth(), mapping()));
        //Not done
    }
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        //accounts.stream().collect(toMap(Account::getFirstName, Account::getBalance));
    }
